...
export class BoardService {
  ...
  toDate(timestamp: firebase.firestore.Timestamp): Date {
                ...
                const modifiedBoards = boards.map((board) => {
                  const modifiedTasks = board.tasks?.map((task) => {
                    if (task.startDate && task.endDate) {
                      task.startDate = this.toDate(
                        task.startDate as unknown as firebase.firestore.Timestamp
                      );
                      task.endDate = this.toDate(
                        task.endDate as unknown as firebase.firestore.Timestamp
                      );
                    }
                    return task;
                  });
