...
export class ProfileService {
  ...
  createUserData(user: User) {
    ...
    const friendsRef: AngularFirestoreDocument<User> = this.db.doc(
      `friends/${user.uid}`
    );
    const userData = {
      uid: user.uid,
    ...
    return (
      ...
      friendsRef.set(friendsData, { merge: true })
    );
  }

  async addFriend(data: Friend[]) {
    const user = await this.afAuth.currentUser;
    return this.db.collection<User>('friends').doc(user?.uid).update({
      friends: data,
    });
  }
  ...
  updateUserName(userId?: string, name?: string) {
    return this.db.collection('users').doc(userId).update({ name });
  }
